...
double get_lumi( int beam_type, int Ee, int Eh, int group, int ev0, int ev1 )
{
    ...
    if ( beam_type == EHE3 )
    {
        if ( Ee == 9 && Eh == 166 )
        {
            double cs[3][2] = {{0.413220600504902, 0.423762615781774}, {3.90464100975673E-02, 4.29147386042907E-02}, {1.26287283201903E-03, 1.58236523230692E-03}};
            double gen_lumi = ev0/(cs[group][0]*(1e-34/1e-43)) + ev1/(cs[group][1]*(1e-34/1e-43));
            return gen_lumi;
        }
        if ( Ee == 10 && Eh == 166 )
        {
    ...
    if ( beam_type == EP || beam_type == BEAM_BG)
    {
        if ( Ee == 18 && Eh == 275 )
        {
            ...
            double cs[4] = {7.42572E+08, 6.93274E+07, 3.36963E+06, 7.94229E+04}; // fb
            // for ( int i = 0; i < 4; i ++ )
            // {
        ...
        if ( Ee == 10 && Eh == 100 )
        {
            ...
            double cs[4] = {5.56009E+08, 3.99867E+07, 1.34265E+06, 6.82136E+03}; // fb
            // for ( int i = 0; i < 4; i ++ )
            // {
            //     double gen_lumi = ev[i][0]/cs[i];
            //     lumi.push_back(gen_lumi);
        ...
        if ( Ee == 5 && Eh == 41 )
        {
            ...
            double cs[3] = {4.00260E+08, 2.03457E+07, 2.86840E+05}; // fb
            // for ( int i = 0; i < 3; i ++ )
            // {
            //     double gen_lumi = ev[i][0]/cs[i];
            //     lumi.push_back(gen_lumi);
